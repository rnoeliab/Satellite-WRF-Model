...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun  8 13:32:06 2020
...
def plot_map(OUTPUT,wrf_file,gas,u10,v10,lat,lon,tiempo,name,unidad):
    ...
    if str(gas.shape) == '(385, 99, 99)':
        print('El gas tiene 3 dimensiones')
        levels = MaxNLocator(nbins=18).tick_values(0,gas.max())
        cmap = cm.get_cmap("Spectral_r",lut=25)
        cmap.set_bad("w")
        norm = BoundaryNorm(levels, ncolors=cmap.N, clip=True)
        for t in range(gas.shape[0]):
#           levels = MaxNLocator(nbins=12).tick_values(gas[t,:,:].min(),gas[t,:,:].max())
...
#           norm = BoundaryNorm(levels, ncolors=cmap.N, clip=True)
            ...
            ax.set_title("Sao Paulo Metropolitan Region" + '\n' +
                 str(name)+ " integrated in the troposphere "+'\n'+
                 " for "+ str(date['local'].dt.strftime('%d-%m-%Y %H:%M').values[t])
                 + " (Local Time)", fontsize=20)
            fig.savefig(OUTPUT+str(name)+'_col_'+str(wrf_file[t][-30:-6])+'.png')
#            plt.show()
            plt.clf()
